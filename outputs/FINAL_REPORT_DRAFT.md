...
본 분석이 추정하고자 하는 반사실(counterfactual)은 다음과 같다. "만약 2022년 5월 28일 신분당선 강남–신사 연장이 개통하지 않았다면, 기존 환승·인접역의 기존 노선 승하차량과 주변 행정동 상권 지표는 통제 단위와 같은 추세를 따랐을 것이다." 이 가정 아래에서 DID 추정치 β는 개통이 실제로 일어난 세계와 개통이 일어나지 않은 반사실 세계 사이의 차이를 측정한다. 핵심은 개통이 아예 일어나지 않는 세계를 직접 관찰할 수 없으므로, 통제 단위의 post-treatment 경로가 처치 단위의 반사실 경로를 근사한다는 평행추세 가정에 의존한다는 점이다.
...
### 2.4 사전 기간 기술통계와 공변량 균형

DID의 내적 타당성을 보강하기 위해 개통 이전(지하철: 2022년 5월 미만, 상권: 2022Q2 미만) 처치군과 통제군의 주요 공변량을 비교한다. 표준화 차이(standardized difference)는 (mean_T − mean_C) / √((SD_T² + SD_C²) / 2)로 계산하며, |std_diff| < 0.25를 대략적 균형 기준으로 본다(Imbens & Rubin, 2015).

**지하철 공변량 균형.** 처치역-노선(4개)의 사전 기간 평균 일일 승하차량은 약 82,853명으로 통제역-노선(21개)의 38,902명보다 약 2.1배 높으며, 표준화 차이는 0.95이다. 이는 강남, 신논현, 논현, 신사가 강남 핵심부에 위치한 대규모 환승역이기 때문에 자연스러운 차이다. 양방향 고정효과(α_s)가 이 수준 차이를 흡수하므로 DID 식별에는 직접적 위협이 되지 않지만, 해석에서 규모 차이를 염두에 두어야 한다. 반면 승차 비율(승차/총승하차)의 표준화 차이는 −0.16으로, 출입 방향 구성은 비교적 균형적이다.

| 공변량 | 처치 평균 | 처치 SD | 통제 평균 | 통제 SD | 표준화 차이 |
|---|---:|---:|---:|---:|---:|
| 일평균 승하차량 | 82,853 | 57,200 | 38,902 | 31,261 | 0.95 |
| 일평균 승차 | 40,894 | 28,650 | 19,093 | 15,442 | 0.95 |
| 일평균 하차 | 41,959 | 28,561 | 19,809 | 15,931 | 0.96 |
| log 일평균 승하차량 | 11.14 | 0.58 | 10.19 | 0.93 | 1.21 |
| 승차 비율 | 0.490 | 0.010 | 0.495 | 0.035 | −0.16 |

**상권 공변량 균형.** 처치 행정동(core 4개)의 사전 기간 분기 평균 매출은 약 2,785억 원으로 통제 행정동(14개)의 1,382억 원보다 약 2.0배 높으며(std_diff = 1.40), 20-30대 매출 비중도 44.5% 대 28.0%로 처치 지역이 크게 높다(std_diff = 2.18). 이는 신사동, 역삼1동 등이 젊은 소비 중심 상권이기 때문이다. 수준 차이는 행정동 고정효과로 흡수되지만, 사전 구성 차이가 크므로 상권 결과의 인과 해석에는 추가적 주의가 필요하다. 반면 주말 매출 비중의 표준화 차이는 0.02로 균형적이다.

| 공변량 | 처치 평균 | 처치 SD | 통제 평균 | 통제 SD | 표준화 차이 |
|---|---:|---:|---:|---:|---:|
| 분기 매출 (억 원) | 2,785 | 1,114 | 1,382 | 878 | 1.40 |
| 분기 거래건수 (만 건) | 882 | 622 | 391 | 191 | 1.07 |
| 주말 매출 비중 | 0.220 | 0.033 | 0.219 | 0.042 | 0.02 |
| 야간 매출 비중 | 0.098 | 0.026 | 0.089 | 0.028 | 0.33 |
| 17-24시 매출 비중 | 0.344 | 0.029 | 0.369 | 0.036 | −0.76 |
| 20-30대 매출 비중 | 0.445 | 0.046 | 0.280 | 0.096 | 2.18 |
| 50-60대+ 매출 비중 | 0.276 | 0.041 | 0.310 | 0.063 | −0.64 |
| 남성 매출 비중 | 0.430 | 0.032 | 0.479 | 0.060 | −1.02 |

## 3. 분석 방법

신분당선 강남–신사 연장은 개통일(2022.05.28)이 명확하고, 처치 단위(기존 환승·인접역)와 비처치 단위(인근 비교역)를 구분할 수 있는 자연실험적 구조를 갖는다. 이에 양방향 고정효과 이중차분법(TWFE DID)을 주 분석 방법으로 선택하였다. DID는 처치·통제 단위 간 시간 불변 차이와 공통 시간 충격을 동시에 제거할 수 있어, 무작위 배정이 불가능한 인프라 충격 분석에 널리 활용된다. 보조적으로 합성통제법(Synthetic Control Method, Abadie et al. 2010)을 적용하였다. SCM은 통제 단위의 가중 조합으로 데이터 기반 반사실을 구성하므로, 평행추세 가정이 완벽하지 않을 때 DID와의 방향 일관성을 점검하는 데 유용하다.
...
**Pretrend 진단.** Event-study의 pre-treatment 계수에 대한 Wald joint test p-value는 사실상 0이다. 이는 pre-treatment 계수들이 joint하게 0과 유의하게 다르다는 뜻이다. 다만 linear pretrend test p-value는 0.77로, pre-treatment에 체계적인 추세가 있었다기보다 개별 월의 노이즈가 joint test를 기각시키는 구조다. 특히 개통 직전인 2022년 1-2월에 +4.5%, +3.7%의 양의 스파이크가 관찰된다. 이 스파이크에서 post로의 급락이 DID 추정치의 일부를 설명할 수 있으므로 mean-reversion 가능성을 배제할 수 없다. 그러나 post 기간 30개월 동안 -10%~-14% 범위의 음의 효과가 안정적으로 유지된다는 점에서, 전체 효과가 artifact일 가능성은 낮다.
...
앞의 §4.6은 같은 통제역 집합에서 통제**변수**를 바꾸는 분석이었다. 여기서는 통제**역 집합** 자체를 바꿔본다.

| 통제역 집합 | 추정 효과 | p-value | N (단위) |
|---|---:|---:|---:|
| 전체 21개 (기본) | -11.02% | 0.057 | 25 |
| 9호선 2-3단계 제외 | -8.05% | 0.192 | 22 |
| Core area만 (13개) | -17.05% | 0.001 | 17 |

통제역 선택에 따라 추정치가 -8%에서 -17%까지, p-value가 0.001에서 0.19까지 변동한다. 9호선 2-3단계 역(언주, 선정릉, 봉은사)을 제외하면 유의성이 사라지는데, 이 역들은 신분당선 연장의 간접 영향권에 있을 수 있다. 반대로 가까운 core area만 남기면 효과가 더 크게 추정된다. 이 민감도는 지하철 결과를 보고할 때 "통제역 정의에 따라 결과가 달라질 수 있다"는 caveat이 필요함을 보여준다.

### 4.8 Permutation inference

본 분석의 treated 역-노선은 4개뿐이다. Clustered SE는 클러스터 수가 적을 때 과소추정되는 것으로 알려져 있다 (Cameron, Gelbach, Miller 2008). 이 문제를 점검하기 위해 25개 역-노선 중 4개를 무작위로 treated로 배정하는 permutation을 999회 반복하고, 실제 DID 계수가 permutation 분포에서 차지하는 위치를 확인했다.

Permutation 결과, 999회 무작위 배정 중 실제 계수(-0.115 log-point)만큼 극단적인 값이 나온 비율은 two-sided p = 0.293, one-sided p = 0.169이다. 이는 conventional clustered p-value(0.057)보다 상당히 높다.

| 추론 방법 | p-value |
|----------|--------:|
| Conventional clustered SE | 0.057 |
| Permutation (two-sided) | 0.293 |
| Permutation (one-sided) | 0.169 |

이 결과는 treated 4개 클러스터에서의 conventional SE가 실제로 과소추정되어 있었음을 보여준다. 따라서 지하철 DID의 통계적 "유의성"에 대해서는 보수적으로 해석해야 한다. 다만 효과의 **방향**(음의 방향)과 **패턴**(출근 하차 > 퇴근 승차 > 야간, 30개월 지속)은 permutation과 무관하게 해석적 가치가 있다.
...
총매출과 거래건수는 증가하지 않았고 통계적으로도 유의하지 않다.
...
위의 anchor/secondary 구분은 각 그룹 내 2개 동의 평균이다. 개별 동 단위로 분해하면 이질성이 크다.

| 그룹 | 행정동 | 총매출 효과 | p | 거래건수 효과 | p |
|---|---|---:|---:|---:|---:|
| anchor | 신사동 | -16.44% | 0.003 | -26.17% | <0.001 |
| anchor | 역삼1동 | -6.11% | 0.225 | +4.43% | 0.140 |
| secondary | 논현1동 | -15.19% | 0.005 | -8.59% | 0.006 |
| secondary | 서초4동 | +0.50% | 0.921 | -9.53% | 0.003 |

신사동이 anchor 효과의 대부분을 지배한다. 역삼1동은 매출이 유의하게 감소하지 않았고 거래건수는 오히려 양의 방향이다. Secondary에서도 논현1동만 매출이 급감했고, 서초4동의 매출 효과는 사실상 0이다(거래건수만 감소). 따라서 "secondary corridor 매출 -7.68%"는 논현1동 한 곳이 만든 숫자이며, 2개 단위의 평균으로 aggregate claim을 하기에는 이질성이 크다.
...
11. Treated 클러스터가 지하철 4개, 상권 4개(서브그룹당 2개)로 적어 conventional clustered SE가 과소추정될 수 있다. 본 보고서에서는 permutation inference를 보조로 제시하여 이 문제를 점검한다.
12. 상권 event-study Wald pretrend test가 모든 outcome에서 p < 10⁻⁸로 기각된다. Parallel trends 가정이 충족되지 않으므로 상권 DID 추정치의 인과 해석에는 본질적 한계가 있다.
13. 상권 treated 그룹 내 개별 행정동 간 이질성이 극단적이다 (§5.4 참조). 2개 단위의 평균 효과를 aggregate claim으로 사용하기에는 표본이 작고 방향이 다르다.
