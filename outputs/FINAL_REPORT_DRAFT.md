...
**상권 공변량 균형.** 상권 패널의 사전 기간 균형 표는 원시 자료 파이프라인 실행 후 `outputs/balance/commerce_pretreatment_balance.csv`에 생성된다. 처치 행정동(core 4개)과 통제 행정동(14개)의 매출 수준, 소비 구성(야간 비중, 20-30대 비중, 주말 비중), 성별 비중을 비교하며, 수준 차이는 행정동 고정효과로 흡수된다.
